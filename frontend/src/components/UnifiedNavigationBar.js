...
const UnifiedNavigationBar = ({ onToggleSidebar, onToggleSplitView, sidebarOpen, splitView }) => {
  ...
  return (
    <div className="bg-dark-800 border-b border-dark-700">
      {/* Tab Bar - Top Row */}
      <div className="h-10 flex items-center px-4 gap-1 border-b border-dark-700">
        <AnimatePresence>
          {tabs.map((tab) => (
            <motion.div
              key={tab.id}
              ...
              className={`flex items-center gap-2 px-3 py-1.5 rounded-t-lg transition-colors cursor-pointer group min-w-0 max-w-[200px] ${
                tab.active 
                  ? 'bg-dark-900 border-t border-l border-r border-dark-600 text-white' 
            ...
            >
              ...
              {/* Close button */}
              <motion.button
                className="opacity-0 group-hover:opacity-100 p-0.5 hover:bg-dark-500 rounded transition-all"
                onClick={(e) => {
                  e.stopPropagation();
                  closeTab(tab.id);
                }}
